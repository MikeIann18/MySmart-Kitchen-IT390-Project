...
async function generateRecipes() {
    ...
    try {
        ...
        // this Cloud Function handles the Gemini request and backup models on the server
    ...
    // If the Cloud Function itself is unreachable (no internet or server down etc.)
    } catch (error) {
        document.getElementById("results-area").innerHTML = `
            <div class="results-error">
...
// Displays the generated recipe results on the page and adds a save button to each recipe card
function displayResults(recipeText) {
    ...
    // Insert the cleaned recipe HTML into the results area
    document.getElementById("results-area").innerHTML = clean;

    // Loop through each recipe card and add an individual save button
    // This lets users save just one recipe instead of all 3 at once
    document.querySelectorAll("#results-area .recipe-card").forEach((card, index) => {
        const saveBtn = document.createElement("button");
        saveBtn.className = "save-recipe-btn add-btn";
        saveBtn.dataset.recipeIndex = index;
        saveBtn.style.margin = "0 24px 20px";
        saveBtn.textContent = "Save This Recipe";
        // Append the save button to the bottom of each recipe card
